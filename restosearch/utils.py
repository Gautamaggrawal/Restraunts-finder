...
class GetRestos:
    ...
    def searchapi(QUERY,start=0,count=20):
        ...
        r = requests.get(EP.format('locations'),
                         headers=HEADER, params={'query': QUERY,"lat":"20.5937","lon":"78.9629"})
        location = r.json()
        # print(location)
        lst = []
        print(location)
        ...
        for loc in rest['restaurants']:
        	loc=loc['restaurant']
        	pppp=populatedb.delay(loc, loc['name'], loc['location']['address'],loc['location']['city'], loc['location']['latitude'],loc['location']['longitude'])
